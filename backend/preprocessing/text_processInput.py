...
def full_preprocessing_pipeline(text, stopwords_set, create_ngrams=True):
    """
    Args:
        text (str): The input text string (e.g., from a 'job_details' column).
...
if __name__ == "__main__":
    ...
    FILE_PATH = os.path.join(BASE_DIR, '../scraping files/jobstreet_techjobs_Jul2025.csv')
    OUTPUT_PATH = 'processed_techjobs_Jul2025.csv'
    ...
    try:
        ...
        df = pd.DataFrame({
            'job_title': ['Data Engineer (Cari Cepat)', 'Frontend Developer needed', 'Dicari: Ahli IT'],
            'job_details': [
                "PT. Sukses Selalu Tbk is seeking a Data Engineer. Responsibilities include building ETL pipelines. Required skills: Python, SQL, and AWS. Gaji pokok + bonus.",
                "We need a developer with experience in React & CSS. Apply now!",
                "Kualifikasi: Lulusan S1 Teknik Informatika, bisa troubleshooting hardware dan software. This job is in Indonesian."
            ]
        })
    ...
    # --- Language Filtering ---
    # drop rows where job_details is not a string
    df.dropna(subset=['job_details'], inplace=True)

    def is_en_or_id(text):
        """Detects if text is English ('en') or Indonesian ('id')."""
        try:
            return detect(text) in ['en', 'id']
        except:
            return False

    print("\nFiltering for English and Indonesian job descriptions...")
    df = df[df['job_details'].apply(is_en_or_id)].reset_index(drop=True)
    print(f"Dataset shape after language filtering: {df.shape}")

    # pipline execution
    print("\nApplying preprocessing pipeline to 'job_details' column...")
    df['processed_terms'] = df['job_details'].apply(
        lambda x: full_preprocessing_pipeline(x, stopwords_set=ALL_STOPWORDS, create_ngrams=True)
    )
    print("Preprocessing complete.")
   
    # analyze processed terms
    print("\n--- Top 20 Most Common Terms Across All Jobs ---")
